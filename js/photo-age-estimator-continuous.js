 ...
 * WITH ADAPTIVE DETECTION FREQUENCY
 * 
 * This is a complete replacement file that includes adaptive detection frequency
 * Simply replace your existing photo-age-estimator-continuous.js with this file
 */
...
(function($) {
    ...
    const ADAPTIVE_CONFIG = {
        ...
        FACE_DETECTED_FRAMES: 1, // Frames with faces before speeding up
        TRANSITION_SPEED: 200,   // Speed of transition between intervals
    ...
    let monitoringTimeout = null; // Changed from interval to timeout for adaptive timing
    ...
    const MONITORING_CONFIG = {
        checkInterval: 100, // Default, but will be overridden by adaptive
    ...
    const photoAgeEstimator = {
        init: function() {
            console.log('Age Estimator Photo Continuous (Adaptive) - Initializing...');
        ...
        initializeElements: function() {
            ...
            adaptiveStatus.style.bottom = '40px';
            adaptiveStatus.style.right = '10px';
            adaptiveStatus.style.padding = '5px 10px';
            adaptiveStatus.style.backgroundColor = 'rgba(0,0,0,0.7)';
            adaptiveStatus.style.color = 'white';
            adaptiveStatus.style.borderRadius = '5px';
        ...
        startCamera: async function() {
            try {
                ...
                video.onloadedmetadata = () => {
                    ...
                    // Start continuous monitoring with adaptive frequency
        ...
        startAdaptiveMonitoring: function() {
            ...
            adaptiveState.currentInterval = ADAPTIVE_CONFIG.MAX_INTERVAL;
            adaptiveState.targetInterval = ADAPTIVE_CONFIG.MAX_INTERVAL;
            adaptiveState.noFaceCount = 0;
            adaptiveState.faceDetectedCount = 0;
            
            // Start the adaptive detection loop
            this.scheduleNextAdaptiveDetection();
        ...
        // NEW: Schedule next detection with adaptive timing
        scheduleNextAdaptiveDetection: function() {
            // Clear any existing timeout
            if (monitoringTimeout) {
                clearTimeout(monitoringTimeout);
            }
            
            // Schedule next detection based on current adaptive interval
            monitoringTimeout = setTimeout(async () => {
                if (detectionActive && !isProcessing) {
                    await this.performAdaptiveDetection();
                }
            ...
            }, adaptiveState.currentInterval);
        },
        
        // NEW: Perform detection and update frequency
        performAdaptiveDetection: async function() {
            const startTime = performance.now();
            adaptiveState.performanceMetrics.totalDetections++;
            
            try {
                // Perform face detection
                const detections = await this.detectFaces();
                
                // Update adaptive frequency based on results
                this.updateAdaptiveFrequency(detections);
                ...
                if (ADAPTIVE_CONFIG.DEBUG_MODE) {
                    console.log(`Detection took ${detectionTime.toFixed(2)}ms, interval: ${adaptiveState.currentInterval}ms`);
                }
                
            } catch (error) {
                console.error('Error in adaptive detection:', error);
            }
        },
        
        // NEW: Detect faces (separated from processing)
        ...
        // NEW: Update adaptive frequency based on detection results
        updateAdaptiveFrequency: function(detections) {
            const facesDetected = detections.length > 0;
            
            if (facesDetected) {
                // Face detected
                adaptiveState.faceDetectedCount++;
                adaptiveState.noFaceCount = 0;
                adaptiveState.performanceMetrics.faceDetections++;
                
                // Speed up if we've detected faces consistently
                if (adaptiveState.faceDetectedCount >= ADAPTIVE_CONFIG.FACE_DETECTED_FRAMES) {
                    this.setTargetInterval(ADAPTIVE_CONFIG.MIN_INTERVAL);
                }
            } else {
                // No face detected
                adaptiveState.noFaceCount++;
                adaptiveState.faceDetectedCount = 0;
                
                // Slow down if no faces for a while
                if (adaptiveState.noFaceCount >= ADAPTIVE_CONFIG.NO_FACE_THRESHOLD) {
                    this.setTargetInterval(ADAPTIVE_CONFIG.MAX_INTERVAL);
                }
            }
            
            // Apply interval changes
            if (ADAPTIVE_CONFIG.ENABLE_SMOOTHING) {
                this.smoothIntervalTransition();
            } else {
                adaptiveState.currentInterval = adaptiveState.targetInterval;
            }
            
            // Update adaptive status display
            this.updateAdaptiveStatus(facesDetected, detections.length);
        },
        
        // NEW: Set target interval
        setTargetInterval: function(interval) {
            if (adaptiveState.targetInterval !== interval) {
                adaptiveState.targetInterval = interval;
                adaptiveState.performanceMetrics.intervalChanges++;
                console.log(`Adaptive: Target interval changed to ${interval}ms`);
            }
        },
        
        // NEW: Smooth transition between intervals
        smoothIntervalTransition: function() {
            if (adaptiveState.currentInterval === adaptiveState.targetInterval) {
                return; // Already at target
            }
            
            // Calculate step size
            const difference = adaptiveState.targetInterval - adaptiveState.currentInterval;
            const step = difference / (ADAPTIVE_CONFIG.TRANSITION_SPEED / 50);
            
            // Start transition if not already running
            if (!adaptiveState.isTransitioning) {
                adaptiveState.isTransitioning = true;
                
                transitionInterval = setInterval(() => {
                    if (Math.abs(adaptiveState.currentInterval - adaptiveState.targetInterval) < Math.abs(step)) {
                        // Reached target
                        adaptiveState.currentInterval = adaptiveState.targetInterval;
                        adaptiveState.isTransitioning = false;
                        clearInterval(transitionInterval);
                        transitionInterval = null;
                    } else {
                        // Step towards target
                        adaptiveState.currentInterval += step;
                        adaptiveState.currentInterval = Math.max(
                            ADAPTIVE_CONFIG.MIN_INTERVAL,
                            Math.min(ADAPTIVE_CONFIG.MAX_INTERVAL, adaptiveState.currentInterval)
                        );
                    }
                }, 50);
            }
        ...
        // NEW: Update adaptive status display
        updateAdaptiveStatus: function(facesDetected, faceCount) {
            // Calculate CPU savings
            const optimalDetections = adaptiveState.performanceMetrics.totalDetections;
            const wouldBeDetections = (Date.now() - adaptiveState.lastDetectionTime) / ADAPTIVE_CONFIG.MIN_INTERVAL;
            const savings = ((wouldBeDetections - optimalDetections) / wouldBeDetections * 100).toFixed(1);
            adaptiveState.performanceMetrics.cpuSaved = Math.max(0, savings);
            
            const status = facesDetected ? 
                `High frequency (${Math.round(adaptiveState.currentInterval)}ms) - ${faceCount} face(s)` : 
                `Low frequency (${Math.round(adaptiveState.currentInterval)}ms) - No faces`;
            
            const adaptiveStatusElement = document.getElementById('age-estimator-adaptive-status');
            if (adaptiveStatusElement) {
                adaptiveStatusElement.innerHTML = `
                    <div>
                        <strong>Adaptive:</strong> ${status}<br>
                        <strong>CPU Saved:</strong> ${adaptiveState.performanceMetrics.cpuSaved}%
                    </div>
                `;
            }
        },
        
        // Modified: Process detections (separated from detection)
        ...
        updateMetricsDisplay: function() {
            ...
            const adaptiveRatio = (ADAPTIVE_CONFIG.MAX_INTERVAL / adaptiveState.currentInterval).toFixed(1);
            metricsHTML += `<br>
                Detection Rate: ${adaptiveRatio}x | 
                CPU Saved: ${adaptiveState.performanceMetrics.cpuSaved}%
            `;
        ...
        stopCamera: function() {
            ...
            // Hide adaptive status
            const adaptiveStatusElement = document.getElementById('age-estimator-adaptive-status');
            if (adaptiveStatusElement) {
                adaptiveStatusElement.style.display = 'none';
            }
            ...
            if (stream) {
                ...
                if (typeof FaceTracker !== 'undefined') {
                    const metrics = FaceTracker.getMetrics();
                    const adaptiveMetrics = adaptiveState.performanceMetrics;
                    this.showMessage(`Session Summary: ${metrics.totalChecks} checks, ${metrics.hitRate}% cache hit rate, ${metrics.apiReduction}% API calls saved, ${adaptiveMetrics.cpuSaved}% CPU saved`);
                }
