...
class AgeEstimator {
    ...
    public function load_includes() {
        ...
        */
        ...
        // Canvas Banner Test Page (Admin only)
        if (is_admin()) {
            add_action('admin_menu', array($this, 'add_canvas_banner_test_page'));
        }
    ...
    public function enqueue_scripts() {
        ...
        // Simple Canvas Banner Ad Script (Display Only)
        wp_enqueue_script(
            'age-estimator-simple-canvas-banner',
            plugin_dir_url(__FILE__) . 'js/simple-canvas-banner.js',
            array('jquery'),
            '2.0.2',
            true
        );
    ...
    private function localize_script() {
        ...
        if (is_user_logged_in()) {
            ...
            // Get user show_results setting
            $user_show_results = get_user_meta($user_id, 'age_estimator_show_results', true);
            ...
            // Get user autostart setting
            $user_camera_autostart = get_user_meta($user_id, 'age_estimator_camera_autostart', true);
            
            // Add showResults parameter with proper handling
            $params['showResults'] = ($user_show_results === '' || $user_show_results === false) ? '1' : $user_show_results;
            
            // Add user meta for frontend access
            $params['userMeta'] = array(
                'cameraAutostart' => $user_camera_autostart === '1',
                ...
                'showResults' => ($user_show_results === '' || $user_show_results === false) ? true : ($user_show_results === '1' || $user_show_results === true)
            );
            ...
            $params['showResults'] = '1'; // Default to showing results for non-logged in users
        }
        
        // Add banner ad settings for JavaScript access
        $params['bannerEnabled'] = get_option('age_estimator_enable_banner_ad', false) ? '1' : '0';
        $params['bannerImage'] = get_option('age_estimator_banner_ad_image', '');
        $params['bannerLink'] = get_option('age_estimator_banner_ad_link', '');
        $params['bannerHeight'] = intval(get_option('age_estimator_banner_ad_height', 100));
        $params['bannerPosition'] = get_option('age_estimator_banner_ad_position', 'bottom');
        $params['bannerOpacity'] = floatval(get_option('age_estimator_banner_ad_opacity', 0.9));
    ...
    /**
     * Add Canvas Banner Test Page to admin menu
     */
    public function add_canvas_banner_test_page() {
        add_submenu_page(
            'options-general.php',
            'Canvas Banner Test',
            'Canvas Banner Test',
            ...
            array($this, 'render_canvas_banner_test_page')
        );
    }
    
    /**
     * Render Canvas Banner Test Page
     */
    public function render_canvas_banner_test_page() {
        echo '<div class="wrap">';
        ...
        echo '<li><code style="background: #fff; padding: 2px 6px; border-radius: 3px;">debugCanvasBanner()</code> - Check banner state</li>';
        echo '<li><code style="background: #fff; padding: 2px 6px; border-radius: 3px;">forceShowCanvasBanner()</code> - Force show banner (TEST MODE)</li>';
        echo '<li><code style="background: #fff; padding: 2px 6px; border-radius: 3px;">reloadCanvasBanner()</code> - 🆕 Reload image from settings</li>';
        echo '<li><code style="background: #fff; padding: 2px 6px; border-radius: 3px;">forceHideCanvasBanner()</code> - Force hide banner</li>';
        echo '</ul>';
        
        echo '<h3>📋 Instructions:</h3>';
        echo '<ol style="margin-left: 20px;">';
        echo '<li>Configure your banner ad in Age Estimator settings first</li>';
        echo '<li>Start the camera below</li>';
        echo '<li>Enter fullscreen mode (double-click camera or use fullscreen button)</li>';
        echo '<li>Canvas banner should appear as an overlay on the camera view</li>';
        echo '</ol>';
        
        echo '<h3>⚙️ Banner Settings:</h3>';
        ...
        echo '<ul style="margin-left: 20px;">';
        echo '<li><strong>Enabled:</strong> ' . ($banner_enabled ? '✅ Yes' : '❌ No') . '</li>';
        echo '<li><strong>Image:</strong> ' . (!empty($banner_image) ? '✅ Configured' : '❌ Not set') . '</li>';
        echo '<li><strong>Height:</strong> ' . $banner_height . 'px</li>';
        echo '<li><strong>Position:</strong> ' . $banner_position . '</li>';
        echo '<li><strong>Opacity:</strong> ' . $banner_opacity . '</li>';
        echo '</ul>';
        ...
        if (!$banner_enabled || empty($banner_image)) {
            ...
            echo '<strong>⚠️ Setup Required:</strong> Please configure your banner ad in the ';
            echo '<a href="' . admin_url('admin.php?page=age-estimator-settings') . '">Age Estimator Settings</a> before testing.';
            echo '</div>';
        }
        
        echo '</div>';
        
        echo '<div style="margin-top: 20px; border: 1px solid #ccc; padding: 20px; background: white; border-radius: 5px;">';
        echo '<h3>🧪 Test Area:</h3>';
        echo do_shortcode('[age_estimator]');
        echo '</div>';
        
        // Add debug mode JavaScript
        echo '<script>';
        echo 'document.addEventListener("DOMContentLoaded", function() {';
        echo '  console.log("🎨 Canvas Banner Test Page Loaded");';
        echo '  // Enable debug mode';
        echo '  const container = document.querySelector(".age-estimator-photo-container");';
        echo '  if (container) {';
        echo '    container.classList.add("age-estimator-debug");';
        echo '    console.log("✅ Debug mode enabled for canvas banner");';
        echo '  }';
        echo '  ';
        echo '  // Add canvas banner state monitor';
        echo '  setInterval(function() {';
        echo '    if (window.simpleCanvasBanner) {';
        echo '      const state = window.simpleCanvasBanner.getState();';
        echo '      const statusEl = document.getElementById("canvas-banner-status");';
        echo '      if (statusEl) {';
        echo '        statusEl.textContent = "Canvas Banner: " + ';
        echo '          (state.bannerVisible ? "VISIBLE" : "HIDDEN") + ';
        echo '          " | Fullscreen: " + (state.isFullscreen ? "ON" : "OFF") + ';
        echo '          " | Camera: " + (state.isCameraActive ? "ACTIVE" : "INACTIVE");';
        echo '        statusEl.style.color = state.bannerVisible ? "green" : "red";';
        echo '      }';
        echo '    }';
        echo '  }, 1000);';
        echo '});';
