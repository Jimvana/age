<?php
/**
 * Add Sound Notification Settings to Age Estimator Admin
 * Include this file in your admin-settings.php after registering settings
 */

// Add settings sections and fields for sound notifications
...
}, 20); // Priority 20 to run after main settings registration
...
function age_estimator_sound_volume_callback() {
    ...
    <input type="range" name="age_estimator_sound_volume" value="<?php echo esc_attr($value); ?>" 
           min="0" max="1" step="0.1" style="width: 200px;" class="age-estimator-volume">
...
// Add JavaScript for media uploader and sound testing
add_action('admin_footer', function() {
    $screen = get_current_screen();
    if ($screen->id !== 'settings_page_age-estimator' && 
        $screen->id !== 'toplevel_page_age-estimator-settings' &&
        $screen->id !== 'age-estimator_page_age-estimator-settings') {
        return;
    }
    ?>
    <script>
