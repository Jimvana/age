...
class AgeEstimatorUserSettings {
    ...
    public function save_user_settings() {
        // Verify nonce
        if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'age_estimator_user_settings')) {
        ...
        $settings = isset($_POST['settings']) ? $_POST['settings'] : $_POST;
        
        // Validate and save face tracking distance
        $face_tracking_key = isset($settings['face_tracking_distance']) ? 'face_tracking_distance' : 'face_match_distance';
        if (isset($settings[$face_tracking_key])) {
            $face_tracking_distance = floatval($settings[$face_tracking_key]);
        ...
        $retail_mode_enabled = isset($settings['retail_mode_enabled']) ? '1' : '0';
        ...
        if ($retail_mode_enabled && !empty($settings['retail_pin'])) {
            $pin = sanitize_text_field($settings['retail_pin']);
            $pin_confirm = sanitize_text_field($settings['retail_pin_confirm']);
        ...
        $age_gating_enabled = isset($settings['age_gating_enabled']) && $settings['age_gating_enabled'] ? '1' : '0';
        update_user_meta($user_id, 'age_estimator_age_gating_enabled', $age_gating_enabled);
        
        // Handle both 'age_gating_threshold' and 'minimum_age' keys
        $threshold_key = isset($settings['age_gating_threshold']) ? 'age_gating_threshold' : 'minimum_age';
        if (isset($settings[$threshold_key])) {
            $age_gating_threshold = intval($settings[$threshold_key]);
        ...
        // Save additional settings that might be present
        $additional_settings = array(
            'camera_autostart' => 'age_estimator_camera_autostart',
            'show_results' => 'age_estimator_show_results', 
            'show_confidence' => 'age_estimator_show_confidence',
            'result_display_time' => 'age_estimator_result_display_time'
        );
        
        foreach ($additional_settings as $setting_key => $meta_key) {
