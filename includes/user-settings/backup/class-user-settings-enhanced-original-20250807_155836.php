<?php
/**
 * Enhanced User Settings Class for Age Estimator Plugin
 * Handles comprehensive frontend user settings panel with advanced features
 * 
...
class AgeEstimatorUserSettingsEnhanced {
        ...
        <div class="age-estimator-settings-enhanced" 
            ...
            <div class="settings-wrapper">
                ...
                <main class="settings-content">
                    <?php if ($atts['show_login_button']): ?>
                    <!-- Mobile User Info (shown when sidebar is hidden) -->
                    <div class="mobile-user-info">
                        <div class="mobile-user-avatar">
                            <?php echo get_avatar(get_current_user_id(), 32, '', '', array('class' => 'mobile-avatar-img')); ?>
                        </div>
                        <div class="mobile-user-details">
                            <span class="mobile-user-name"><?php echo esc_html(wp_get_current_user()->display_name); ?></span>
                            <span class="mobile-user-role"><?php echo esc_html(ucfirst(wp_get_current_user()->roles[0] ?? 'User')); ?></span>
                        </div>
                        <a href="<?php echo wp_logout_url(get_permalink()); ?>" class="mobile-logout-btn">
    ...
    public function save_user_settings() {
        ...
        foreach ($settings as $key => $value) {
            ...
            } elseif (in_array($key, array('show_results', 'show_confidence', 'age_gating_enabled', 
    ...
    private function render_login_form($atts) {
        ...
        /* Dark theme support */
        .age-estimator-login-wrapper[data-theme="dark"] .login-container {
            background: #1f2937;
            color: #f9fafb;
        }
        
        .age-estimator-login-wrapper[data-theme="dark"] .login-notice {
            background: #374151;
            border-color: #4b5563;
        }
        
        .age-estimator-login-wrapper[data-theme="dark"] .notice-text h3 {
            color: #f9fafb;
        }
        
        .age-estimator-login-wrapper[data-theme="dark"] .notice-text p,
        .age-estimator-login-wrapper[data-theme="dark"] .login-features,
        .age-estimator-login-wrapper[data-theme="dark"] .login-features ul {
            color: #d1d5db;
        }
        
        .age-estimator-login-wrapper[data-theme="dark"] .login-features h4 {
            color: #f9fafb;
        }
